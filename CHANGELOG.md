...
- **Phase 01 CloudAuth — missing IPC handlers wired** (`electron/ipc-handlers/cloud-auth-ipc-handlers.ts`, `electron/preload/handlers-auth.ts`): Added 8 missing IPC handlers for CloudAuth SDK methods: `signInWithGoogleIdToken`, `registerWithEmail`, `verifyEmailAddress`, `resetPassword`, `completePasswordReset`, `registerTrustedDevice`, `listTrustedDevices`, `removeTrustedDevice`. All wired through `window.cloudAuthSdk` preload bridge. `@soostori/auth` updated from `^0.1.0-alpha.7` to `^0.1.0-alpha.9`.

- **Phase 01 OperationalAuth enrollment + recovery IPC** (`electron/auth/desktop-operational-auth.ts`, `electron/ipc-handlers/auth-handlers.ts`, `electron/preload/handlers-db.ts`, `electron/preload/ipc-signatures-db.ts`): Full `@soostori/auth` OperationalAuth SDK wiring for device enrollment, PIN setup/verify/change/clear, offline entitlement, session serialization, and PIN recovery flow. Added `cloudApi()` casting helper to bridge `FIDScriptAuthApiClient` → `OperationalCloudApi`. All 13 new IPC handlers registered: `op:getEnrollmentState`, `op:beginEnrollment`, `op:completeEnrollmentWithCloudVerify`, `op:changePin`, `op:clearPin`, `op:getLockState`, `op:isWithinOfflineEntitlement`, `op:isSessionExpired`, `op:serializeSession`, `op:deserializeSession`, `op:requestPinRecovery`, `op:verifyPinRecoveryCode`, `op:resetPinWithRecovery`. Exported `SetupPinResult` and `VerifyPinResult` interfaces from `desktop-operational-auth.ts`. Updated `@soostori/auth` to `^0.1.0-alpha.7`.
...
## 2026-09-04 — phase-924-customers-debts

### Customers + Debts — Phase 9.2.4

**SDK adapter (`packages/desktop-adapter/src/`):**
- `customers-repository.ts` — completed with `findById`, `findMany({isActive})`, `create`, `update`, `softDelete` over `customers` table. Maps `is_active` → `isActive`. `shopId` is `null` (no shop column in Desktop schema).
- `debts-repository.ts` — new file. Implements `DebtsRepository` over `debts` + `debt_payments` tables with `findById`, `findMany({customerId,status})`, `createDebt`, `recordPayment`, `findPayments`. Defines local `Debt` and `DebtPayment` types. `recordPayment` inserts into `debt_payments` and updates `debts.amount_paid` + `debts.status` in one transaction.
- `domain-repositories.ts` — exports `DebtsRepository`
- `packages/core/src/ids.ts` — added `asDebtId` and `asDebtPaymentId` cast helpers

**Desktop IPC handlers (`electron/ipc-handlers/`):**
- `customer-handlers.ts` — `db:customers:create` now calls `CustomersRepository.create()`, then returns raw row for backward compatibility. Handler initializes adapter via `setDatabase(getDatabase())`.
- `debt-handlers.ts` — `db:debts:create` calls `DebtsRepository.createDebt()`. `db:debts:recordPayment` calls `DebtsRepository.recordPayment()`. Both return raw rows for backward compatibility.
