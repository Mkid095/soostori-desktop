...
- **Cloud service scaffold** (`electron/services/cloud-service.ts`): `verifySubscription()`, `reportHeartbeat()`, `checkCloudHealth()`, `isUnverifiedTooLong()` — all stubs with TODO comments for Phase 3 fidscript_api wiring. `electron/services/cloud-sync-service.ts`: `pushQueuedEvents()` / `pullCloudEvents()` periodic background sync stubs. `electron/services/heartbeat-service.ts`: fires heartbeat every 5 min, throttled, started in `main.ts` after window creation. Subscription verification fires on startup (fire-and-forget). Architecture decision: InstantDB rejected per `docs/ARCHITECTURE.md` — cloud sync defers to Phase 3 Go backend.
