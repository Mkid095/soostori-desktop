...
const MpesaPaymentView: React.FC<MpesaPaymentViewProps> = ({
  ...
  const labelKey =
    method === 'sendMoney' ? 'pos.sendMoneyTo' :
    method === 'mpesaPaybill' ? 'set.mpesaPaybill' :
    method === 'bankPaybill' ? 'set.bankPaybill' : 'set.pochiLaBiashara'
    ...
    <div className="space-y-4 p-5 bg-green-50 dark:bg-green-950/40 rounded-2xl border border-green-100 dark:border-green-900/50 transition-colors duration-200">
      ...
      {method === 'sendMoney' && shopSettings?.mpesaSendMoneyPhone && (
        <div className="bg-white dark:bg-slate-900 rounded-xl p-4 text-center border border-green-200 dark:border-green-800 transition-colors duration-200">
          <p className="text-xs text-green-600 dark:text-green-400 font-semibold mb-1">{t('pos.phoneNumber')}</p>
      ...
      {method === 'mpesaPaybill' && (
        <div className="bg-white dark:bg-slate-900 rounded-xl p-4 text-center border border-green-200 dark:border-green-800 transition-colors duration-200">
          <p className="text-xs text-green-600 dark:text-green-400 font-semibold mb-1">{t('pos.paybillNumber')}</p>
          ...
          {shopSettings?.mpesaPaybillAccount && (
            <>
              <p className="text-xs text-green-600 dark:text-green-400 font-semibold mb-1 mt-2">{t('pos.accountNumber')}</p>
      ...
      {method === 'bankPaybill' && (
        <div className="bg-white dark:bg-slate-900 rounded-xl p-4 text-center border border-green-200 dark:border-green-800 transition-colors duration-200">
          <p className="text-xs text-green-600 dark:text-green-400 font-semibold mb-1">{t('set.bankPaybill')}</p>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl p-4 text-center border border-green-200 dark:border-green-800 transition-colors duration-200">
        <p className="text-xs text-green-600 dark:text-green-400 font-semibold mb-1">{t('pos.customerPays')}</p>
      ...
      {!mpesaConfirmed ? (
        ...
        <div className="p-4 bg-emerald-100 dark:bg-emerald-950/40 rounded-xl text-center border-2 border-emerald-300 dark:border-emerald-700 transition-colors">
          ...
          <p className="text-sm font-bold text-emerald-700 dark:text-emerald-300">{t('pos.paymentConfirmed')}</p>
          <p className="text-xs text-emerald-600 dark:text-emerald-400 mt-1">{formatCurrency(total)} {t('pos.received')}</p>
        </div>
      )}
    </div>
  )
