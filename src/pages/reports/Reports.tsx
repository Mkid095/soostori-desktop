...
const Reports: React.FC = () => {
  ...
  useEffect(() => { setDisplayedCount(PAGE_SIZE); setPage(0) }, [dateFilter, paymentFilter, search])
  ...
  return <div className="flex h-full flex-col overflow-hidden bg-bg-primary transition-colors duration-200">
    ...
    <div className="flex-1 overflow-y-auto">{isLoading ? <div className="flex justify-center py-20"><RefreshCw className="animate-spin text-brand-orange" /></div> : filteredSales.length === 0 ? <div className="py-20 text-center text-sm font-bold text-slate-400">No sales found</div> : <>
      {pagedSales.map((sale) => { const meta = methodMeta(sale.paymentMethod); const Icon = meta.icon; return <button key={sale.id} onClick={() => setSelectedSale(sale.id)} className="flex w-full items-center gap-3 border-b border-slate-100 bg-bg-secondary px-4 py-3 text-left dark:border-slate-700"><div className={`flex h-8 w-8 items-center justify-center rounded-lg ${meta.bg}`}><Icon size={14} className={meta.color} /></div><div className="min-w-0 flex-1"><div className="flex justify-between"><b className="text-sm text-slate-800 dark:text-slate-100">{formatCurrency(sale.totalAmount)}</b><span className={`rounded px-1.5 py-0.5 text-[10px] font-bold ${meta.bg} ${meta.color}`}>{meta.label}</span></div><p className="truncate text-[10px] text-slate-400">{sale.items_summary || 'No items'}</p>{sale.note && <p className="text-[10px] italic text-slate-400 truncate">Note: {sale.note}</p>}</div><div className="text-right text-[10px] text-slate-400"><p>{new Date(sale.createdAt).toLocaleDateString()}</p><p>{new Date(sale.createdAt).toLocaleTimeString()}</p></div></button> })}
