...
import type { ExpenseCategory } from './AddExpenseSheet'
...
interface AddRecurringSheetProps {
  onSave: (data: { amount: number; category: string; frequency: string; nextDueDate: string }) => Promise<void>
  onClose: () => void
  isSaving: boolean
}

const AddRecurringSheet: React.FC<AddRecurringSheetProps> = ({ onSave, onClose, isSaving }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (!amount) return
    onSave({ amount: parseFloat(amount), category, frequency, nextDueDate })
  }
      ...
      <div className="bg-white dark:bg-bg-secondary w-full rounded-t-3xl shadow-2xl animate-slide-up transition-colors duration-200">
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100 dark:border-slate-700">
          <p className="font-black text-slate-800 dark:text-slate-100 text-sm">
            {t('exp.addRecurring') || 'Add Recurring Expense'}
          </p>
