    ...
    <div className="flex gap-3 mt-5 pt-4 pb-4 border-t border-slate-100 dark:border-slate-700">
      {step > 0 ? (
        ...
        <button type="button" onClick={onCancel}
          className="flex-1 py-3.5 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-xl font-bold text-sm hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors">
          Cancel
        </button>
      )}
      {isLastStep ? (
        <button type="submit" disabled={isSaving || !isValid}
          className="flex-1 flex items-center justify-center gap-2 py-3.5 bg-brand-orange text-white rounded-xl font-bold text-sm hover:bg-orange-600 disabled:opacity-50 transition-colors shadow-md shadow-orange-200/60 dark:shadow-orange-900/30">
