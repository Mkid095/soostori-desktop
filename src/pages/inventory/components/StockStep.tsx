  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <FormField label="Opening Stock Qty / Kiasi cha Hisa">
          <input
            type="number"
            min="0"
        ...
        <FormField label="Low Stock Alert / Tahadhari ya Hisa">
          <input
            type="number"
            min="0"
      ...
      <div className="flex items-center gap-3 p-3 bg-white dark:bg-slate-800 rounded-xl border border-border-color dark:border-slate-700">
        <button
          ...
          className={`relative inline-flex h-6 w-11 items-center rounded-full
            transition-colors shrink-0 ${form.trackInventory ? 'bg-brand-orange' : 'bg-slate-300 dark:bg-slate-600'}`}
          style={{ minWidth: '44px', minHeight: '24px' }}
        >
          <span className={`inline-block h-4 w-4 transform rounded-full bg-white shadow
            transition-transform ${form.trackInventory ? 'translate-x-5' : 'translate-x-1'}`} />
        </button>
        <div>
          <p className="font-bold text-sm text-text-primary dark:text-slate-100">
            Track Inventory / Fuatilia Hisa
          </p>
      ...
      <div className="border-t border-slate-200 dark:border-slate-700 pt-4 mt-4">
        <p className="text-xs font-bold text-text-muted dark:text-slate-400 uppercase tracking-wider mb-3">
